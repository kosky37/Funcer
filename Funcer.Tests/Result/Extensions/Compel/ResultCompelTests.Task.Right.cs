...
public class ResultCompelTests_Task_Right
{
    ...
    {
        await Assert.ThrowsAsync<ArgumentException>(() => TestResult.Failure.Compel(_ => AsyncFunc.Returns.ArgumentException));
    }
    
    [Fact]
    public async Task Should_Not_Throw_Custom_When_Result_Is_Success()
    {
        await TestResult.Success.Compel(_ => AsyncFunc.Returns.ArgumentException);
    }
}