...
public static class ResultExtensions_Map
{
    ...
    {
        return result.IsFailure ? Result<TValue>.Failure(result.Errors) : next();
    }
    ...
    {
        return result.IsFailure ? Result<TValue>.Failure(result.Errors) : Result.Success(next());
    }
}