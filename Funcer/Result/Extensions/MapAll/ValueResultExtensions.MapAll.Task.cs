...
{
    public static async Task<Result<IEnumerable<TValue2>>> MapAll<TValue, TValue2>(this Task<Result<IEnumerable<TValue>>> resultTask, Func<TValue, Task<Result<TValue2>>> next)
    {
    ...
    public static async Task<Result<IEnumerable<TValue2>>> MapAll<TValue, TValue2>(this Task<Result<IEnumerable<TValue>>> resultTask, Func<TValue, Task<TValue2>> next)
    {
        var result = await resultTask;
        if (result.IsFailure)
        {
