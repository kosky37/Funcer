...
public static partial class ValueResultExtensions
{
    ...
    {
        public Task Resolve(Func<TValue, Task> onSuccess, Func<IEnumerable<ErrorMessage>, Task> onFailure)
        {
            if (result.IsFailure) return onFailure(result.Errors);
            else return onSuccess(result.Value!);
        }

        public Task Resolve(Func<TValue, Task> onSuccess, Action<IEnumerable<ErrorMessage>> onFailure)
        {
        ...
        public Task Resolve(Action<TValue> onSuccess, Func<IEnumerable<ErrorMessage>, Task> onFailure)
        {
            if (result.IsFailure) return onFailure(result.Errors);
        ...
        public Task Resolve(Func<TValue, IEnumerable<WarningMessage>, Task> onSuccess, Func<IEnumerable<ErrorMessage>, Task> onFailure)
        {
            if (result.IsFailure) return onFailure(result.Errors);
            else return onSuccess(result.Value!, result.Warnings);
        }

        public Task Resolve(Func<TValue, IEnumerable<WarningMessage>, Task> onSuccess, Action<IEnumerable<ErrorMessage>> onFailure)
        {
            if (result.IsFailure) onFailure(result.Errors);
            else return onSuccess(result.Value!, result.Warnings);
            return Task.CompletedTask;
        ...
        public Task Resolve(Action<TValue, IEnumerable<WarningMessage>> onSuccess, Func<IEnumerable<ErrorMessage>, Task> onFailure)
        {
            if (result.IsFailure) return onFailure(result.Errors);
        ...
        public Task Resolve(Func<TValue, Task> onSuccess, Func<Task> onFailure)
        {
            if (result.IsFailure) return onFailure();
            else return onSuccess(result.Value!);
        }

        public Task Resolve(Func<TValue, Task> onSuccess, Action onFailure)
        {
        ...
        public Task Resolve(Action<TValue> onSuccess, Func<Task> onFailure)
        {
            if (result.IsFailure) return onFailure();
            else onSuccess(result.Value!);
            return Task.CompletedTask;
        ...
        public Task Resolve(Func<TValue, IEnumerable<WarningMessage>, Task> onSuccess, Func<Task> onFailure)
        {
            if (result.IsFailure) return onFailure();
            else return onSuccess(result.Value!, result.Warnings);
        }

        public Task Resolve(Func<TValue, IEnumerable<WarningMessage>, Task> onSuccess, Action onFailure)
        {
            if (result.IsFailure) onFailure();
            else return onSuccess(result.Value!, result.Warnings);
            return Task.CompletedTask;
        ...
        public Task Resolve(Action<TValue, IEnumerable<WarningMessage>> onSuccess, Func<Task> onFailure)
        {
            if (result.IsFailure) return onFailure();
            else onSuccess(result.Value!, result.Warnings);
            return Task.CompletedTask;
